...
# Custom logic to modify all sheets in the Excel file (as per your logic)
...
def main():
    ...
    # Dropdowns for selecting workbooks and date ranges
    ...
    if selected_workbook:
        ...
        if not nav_data.empty:
            ...
            # Automatically trigger the Excel modification code whenever a dropdown is selected
            if st.button("Run Excel Modification"):
                workbook = openpyxl.load_workbook(os.path.join(WORKBOOK_DIR, selected_workbook))
                ...
                with open(os.path.join(WORKBOOK_DIR, selected_workbook), 'wb') as f:
                    f.write(modified_content.read())
                st.success(f"Successfully modified and saved {selected_workbook}!")
