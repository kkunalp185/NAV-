...
# Define the directory where the workbooks are stored (NAV folder in the repo)
WORKBOOK_DIR = "NAV"  # This should point to the NAV folder in your GitHub repo
...
# --- CODE FOR MODIFYING EXCEL SHEETS ---
# Function to download an Excel file from the NAV folder
...
def save_excel_to_memory(workbook, filename):
    output = BytesIO()
    workbook.save(output)
    output.seek(0)

    # Save locally to the NAV folder
    with open(os.path.join(WORKBOOK_DIR, filename), 'wb') as f:
        f.write(output.read())
    st.success(f"Successfully modified and saved {filename}!")
...
def main():
    ...
    if selected_workbook:
        ...
        if not nav_data.empty:
            ...
            workbook = download_excel_file(selected_workbook)
            modified_workbook = modify_all_sheets(workbook)
            save_excel_to_memory(modified_workbook, selected_workbook)
